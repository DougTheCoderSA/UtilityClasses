...
namespace UtilityClasses
{
    ...
    public class IniFile
    {
        ...
        public void Load(string sFileName, bool bMerge)
        {
            ...
            oReader.Close();
            FileLoaded = true;
            // Get the current file timestamp and save it so we know when to reload the ini file
            FileVersionLastLoaded = File.GetLastWriteTime(sFileName);
        ...
        public void Save(string sFileName)
        {
            ...
            oWriter.Close();
            // Why set the file loaded info here? Because we now have an on-disk file (whether
            // or not we had one before) and its contents should match the in-memory structure.
            // We can use this to know whether we need to update the file or not.
            FileLoaded = true;
            // Get the current file timestamp and save it so we know when to reload the ini file
            FileVersionLastLoaded = File.GetLastWriteTime(sFileName);
            FileLoadedPath = sFileName;
        ...
        // Check if a KeyValue exists ## ADDED BY DOUG ##
        public bool IsKeyValuePresent(string sSection, string sKey)
        {
            IniSection s = GetSection(sSection);
            if (s != null)
            {
        ...
        // Gets a KeyValue, adding a default value if it doesn't exist  ## ADDED BY DOUG ##
        /// <summary>
        /// Gets a KeyValue, adding a default value if it doesn't exist. If the default value was
        /// used, this is indicated by setting the reference parameter DefaultUsed to true.
        /// </summary>
        /// <param name="sSection">The section.</param>
        /// <param name="sKey">The key.</param>
        /// <param name="sDefaultValue">The default value to use if the value does not yet exist in the ini file.</param>
        /// <param name="DefaultUsed">Boolean reference variable that indicates if the default value was used.</param>
        /// <returns>Value from the ini file or the default if not found</returns>
        public string GetKeyValueOrDefault(string sSection, string sKey, string sDefaultValue, ref bool DefaultUsed)
        {
            if (IsKeyValuePresent(sSection, sKey))
            {
                return GetKeyValue(sSection, sKey);
            }
            else
            {
                SetKeyValue(sSection, sKey, sDefaultValue);
                ValueLastChanged = DateTime.Now;
                DefaultUsed = true;
        ...
        public class IniSection
        {
            ...
            public IniKey AddKey(string sKey)
            {
                ...
                if (sKey.Length != 0)
                {
                    ...
                    else
                    {
                        ...
                        m_pIniFile.ValueLastChanged = DateTime.Now;
                        m_keys[sKey] = k;
                    }
                }
                return k;
            ...
            public class IniKey
            {
                ...
                public void SetValue(string sValue)
                {
                    if (m_sValue != sValue)
                    {
                        m_section.m_pIniFile.ValueLastChanged = DateTime.Now;
                    }
                    m_sValue = sValue;
                }
